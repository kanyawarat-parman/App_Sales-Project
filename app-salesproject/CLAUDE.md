...
-- ไม่อยู่ใน sql/schema.sql — โครงสร้างเต็มอยู่ที่ sql/add_kpi_settings.sql (เดิม api/kpi_settings.php สร้างเอง เอาออกแล้ว 2026-09-26)
...
1. **ทุกตารางต้องมี 4 ช่อง audit เสมอ:** `created_by` (ผู้สร้าง), `created_at` (วันเวลาที่สร้าง), `updated_by` (ผู้แก้ไข), `updated_at` (วันเวลาที่แก้ไข)
   ...
   - การแก้ที่ระบบทำเอง (คำนวณ SLA, เวลาส่ง LINE/Email, `last_login`, หน้าที่กำลังใช้) ไม่บันทึก `updated_by` และคง `updated_at` เดิม (`SET ..., updated_at = updated_at`) ไม่งั้นเวลาแก้ไขล่าสุดจะเลื่อนทั้งที่ไม่มีคนแก้
   - บันทึกหลายรายการพร้อมกันด้วย `INSERT ... ON DUPLICATE KEY UPDATE` (เป้าหมาย/KPI/วันหยุด/สร้างเวรทั้งปี) เปลี่ยน `updated_by` เฉพาะแถวที่ค่าเปลี่ยนจริง: `updated_by = IF(col <=> VALUES(col), updated_by, VALUES(updated_by))` วางไว้**ก่อน**ช่องที่แก้ เพราะ MySQL ทำจากซ้ายไปขวา
...
3. **โค้ดหน้าเว็บ/API ห้ามสร้างหรือแก้โครงสร้างตาราง (ยืนยันจากผู้ใช้ 2026-09-26)** — ห้ามมี `CREATE TABLE` / `ALTER TABLE` / `DROP TABLE` (รวม `CREATE TABLE IF NOT EXISTS`) ในไฟล์ `api/*.php` / `includes/*.php` — โครงสร้าง DB ทุกอย่างอยู่ในไฟล์ `sql/` เท่านั้น ให้คนรันเองอย่างตั้งใจหลังผู้ใช้อนุญาต
   - เหตุผล: (1) โครงสร้างอยู่ที่เดียว ไม่เกิดกรณีแก้ที่หนึ่งแล้วอีกที่ค้างเป็นแบบเก่า (2) ไม่มีการแตะ DB โดยไม่ได้รับอนุญาต (3) user DB ของหน้าเว็บควรมีแค่สิทธิ์อ่าน/เขียนข้อมูล ไม่ต้องมีสิทธิ์สร้างตาราง (4) ไม่รันคำสั่งเกินทุกครั้งที่เปิดหน้า
   - บทเรียนจริง: `api/targets.php` / `api/kpi_settings.php` เคยสร้างตารางเองตอนเปิดหน้า โครงสร้างในโค้ดไม่มีช่อง audit/COMMENT ค้างเป็นแบบเก่าหลังเพิ่ม audit ให้ตารางจริง ถ้าติดตั้งบน DB ใหม่จะได้ตารางขาดช่องและบันทึกไม่ได้ — เอาออก 2026-09-26 ย้ายไปอยู่ที่ `sql/add_user_monthly_targets.sql` / `sql/add_kpi_settings.sql`
   - ไฟล์ `sql/` สำหรับตารางใหม่ต้องมีโครงสร้างครบตามจริง (COMMENT ไทย + ช่อง audit + FK) และเขียนแบบรันซ้ำได้ (`IF NOT EXISTS` / เช็คก่อนเพิ่ม-ลบ) — แก้ตารางจริงเมื่อไร ต้องแก้ไฟล์ `sql/` ของตารางนั้นให้ตรงด้วย
   - ค่าเริ่มต้นที่หน้าเว็บต้องใช้ถ้ายังไม่มีในตาราง (เช่น `$defaults` ใน `api/kpi_settings.php`) ใส่ในโค้ดได้ — ห้ามแค่การสร้าง/แก้โครงสร้าง
